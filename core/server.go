...
func (server *Server) Connect() {
	...
	if err != nil {
		if ErrorAssert(err, "ssh: unable to authenticate") {
			Printer.Errorln("connect fail，unable to authenticate")
			Log.Category("server").Error("connect fail，unable to authenticate", err)
			return
		}
...
func (server *Server) Edit() {
	...
	Printer.Info("Ip(default=" + server.Host + ")：")
	fmt.Scanln(&input)
	if input != "" {
		server.Host = input
		input = ""
	...
	Printer.Info("User(default=" + server.Name + ")：")
	fmt.Scanln(&input)
	if input != "" {
		server.Name = input
		input = ""
